...
Честно - из-за наличия erase стало делать лень. Объясняю. Чтобы фигуры изменять каждую отдельно, их надо хранить отдельно как фигуры. Но чтобы делать erase надо в обязательном порядке все преобразовывать в картинку.
Но в целом планировалось так - Canvas.Record это объект в котором есть и картинка и фигуры (массив). Пока мы рисуем фигуры, картинка пустая, и только фигуры добавляются в массив. Все это тогда поддерживает историю нормально. Но как только мы воспользовались ластиком, то все фигуры преобразуются в картинку, и дальше их нельзя уже поменять. Правда в истории, при этом фигуры остаются (можно отменой найти), ну и все новые фигуры нарисованные, опять же будут доступны.

P.S. Под фигурой понимается: набор точек, тип фигуры, цвет, толщина линии, размер, угол поворота, смещение. Произвольная линия, тоже фигура.
P.P.S. Такая технология оптимальнее с точки зрения хранения в памяти. Генерацию кадров можно былобы сделать прям вообще огромную и быстрее...  
P.P.P.S. Стояло бы задание сделать изменение фигур отдельно, и где-нибудь среди первых позиций по балам, я бы сделал, а так... Ну может хранение кадра еще и переделаю...
...
* 2 часа - добавление FigurePainter-а и завязка его пока на старый рекорд. Да и сам он не оптимизирован.
